...
const bool decimal[4] = {0,1,1,1}; // assigns the decimal point to the right pin
...
int main() {
    ...
    const float VREF = 3.3f; // for voltage scaling

    while (true) {
        uint16_t raw = adc_read();               // reads adc as a 12-bit value
        float voltage = (raw * VREF) / 4095.0f;  // convert to volts
        int mv = (int)(voltage * 1000 + 0.5);    // convert to millivolts (because its easier to use bc it has 4 digits
        if (mv > 3300) mv = 3300;                // keeps it from going above 3.3 V
